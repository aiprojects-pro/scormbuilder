...
def _drawing_alt_text(run_el) -> str:
    ...
    v0.5.11: filtra textos automáticos que Word/Copilot inyecta como descr
    cuando la imagen viene de una tabla con contenido generado por IA. Estos
    textos son SIEMPRE el mismo aviso ("El contenido generado por IA puede ser
    incorrecto") y no aportan información sobre la imagen — son ruido que
    contamina el alt-text y el caption del bloque.
    """
    drawing = run_el.find(f"{W}drawing")
    if drawing is None:
        return ""
    ...
    for el in drawing.iter():
        ...
        if tag == "docPr":
            ...
            descr = descr.strip()
            if descr and not _is_ai_disclaimer_noise(descr):
                return descr
    return ""


# v0.5.11: textos automáticos que Word/Copilot inyecta como alt-text
# y que NO deben usarse como descripción de la imagen.
_AI_NOISE_PATTERNS = (
...
def _is_ai_disclaimer_noise(text: str) -> bool:
    """Detecta si un alt-text es el aviso automático de Word/Copilot.
    Soporta variantes con prefijos (Tabla, Image, Figura) + texto basura.
    """
    t = (text or "").strip().lower()
    if not t:
        return False
    return any(p in t for p in _AI_NOISE_PATTERNS)
